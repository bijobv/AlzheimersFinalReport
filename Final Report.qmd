...
rand.forest.function <- function(x) { 
  ...
  CM <- confusionMatrix(test_diagnosis, predicted_labels, positive = "Alzheimers", mode = "everything")
...
rfplot <- ggplot(perf_measures_long, aes(x = factor(ntrees), y = Value, fill = Measure)) +
  ...
  scale_fill_manual(values = c("#E09F3E", "#335C67", "#E3DED1", "#99A88C")) + 
  theme_minimal()
```
...
Our primary goal in this business case is **to develop a right-sized predictive model with adequate performance** capable of classifying individuals as having or not having Alzheimer’s based on clinical patient data. Additionally, we aim to identify the specific factors or combinations of factors that can reliably aid in predicting Alzheimer’s disease. Lastly, we will address challenges related to the chosen dataset to improve the quality of our model.
...
To prepare the dataset for training and testing, the raw data was transformed into a consistent format and saved as an RDS file. All features were converted to numeric values and standardized to have a mean of 0 and a standard deviation of 1. The target variable was converted into a factor with two levels: 'Alzheimer' and 'No Alzheimer'. This standardization ensures that all classifiers use the same dataset for training, testing, and performance evaluation. Other feature engineering techniques are discussed further in this report. It is noteworthy that the predictor variables "EducationLevel" and "Ethnicity" were dummy encoded to maintain the conversion to a numeric format. This is especially important for models like kNN, LDA and Lasso that require a numeric format.
...
-   **LDA**: No hyperparameter tuning required.
...
resamples_results <- resamples(list(kNN = knn_model, LDA = lda_model, RF = rf_model2, Lasso = lasso_model))
...
```{r CPE2, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=6}
...
CM_long <- CM_allmodels %>%
  pivot_longer(
    cols = c("Accuracy", "Recall", "Precision", "F1"),
    names_to = "Metric",
    values_to = "Value"
  )
...
##### Figure 6. Model performance across all probability thresholds

```{r CPE3, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=6}
predict_rf2 <- predict(rf_model2, newdata = test_data, type = "prob")
...
custom_colors <- c("#E09F3E", "#335C67", "#E3DED1", "#99A88C")
...
##### Figure 7. Random forest Model performance across ensemble of trees tested
